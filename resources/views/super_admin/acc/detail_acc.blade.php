...
@section('content')
    ...
    {{-- =========================
    MODUL PERT INPUT WAKTU
    ========================= --}}

    @if (!empty($acc['tgl_pengerjaan']) && $acc['tgl_pengerjaan'] != '-')
        @if (empty($acc['pertResult']))
            <div class="rekap-section mt-6">
                <h3 class="section-title">Input Waktu</h3>

                <div class="rekap-box">

                    <form action="{{ route('superadmin.pert.store', $acc['id']) }}" method="POST">
                        @csrf
                        ...
                        <table class="pending-table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:150px">Waktu Tercepat (Hari)</th>
                                    <th style="width:150px">Waktu Normal (Hari)</th>
                                    <th style="width:150px">Waktu Terlama (Hari)</th>
                                </tr>
                            </thead>
                            ...
                            <tbody>
                                ...
                                @foreach ($acc['pertTasks'] as $i => $task)
                                    <tr>
                                        ...
                                        <input type="hidden" name="id_master[]" value="{{ $task['id_master'] }}">

                                        <td style="text-align:center">
                                            <input type="number" name="optimis[]" class="input-pert" step="any"
                                                min="0.5" required>
                                        </td>

                                        <td>
                                            {{ $task['realistis'] }}
                                            <input type="hidden" name="kode[]" value="{{ $task['kode'] }}">
                                            <input type="hidden" name="realistis[]" value="{{ $task['realistis'] }}">
                                        </td>

                                        <td style="text-align:center !important">
                                            <input type="number" name="pesimis[]" class="input-pert" step="any"
                                                min="0.1" required>
                                        </td>
        ...
        @else
            <div class="rekap-section mt-6">
                <h3 class="section-title">Acuan Urutan Pekerjaan Berdasarkan Model Terpilih</h3>
                    ...
                    <table class="pending-table">
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>Nama Pekerjaan</th>
                                <th style="width:180px">Durasi Maksimal (Hari)</th>
                            </tr>
                        </thead>

                        <tbody>

                            @foreach ($acc['pertResult'] as $i => $task)
                                <tr>
    ...
    @if (!empty($acc['allModels']) && count($acc['allModels']) > 0)
        <div class="rekap-section mt-6">
